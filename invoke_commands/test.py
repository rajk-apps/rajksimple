...
def test(c, option="", html=False, xml=False, notebook_tests=True):
    ...
    if notebook_tests:
        new_test_scripts = []
        for nb_idx, nb_file in enumerate(glob.glob("{}/*.ipynb".format(doc_notebooks_dir))):
            nb_dic = json.load(open(nb_file))
            nb_code = '\n'.join(['\n'.join(c["source"]) for c in nb_dic["cells"] if (c["cell_type"] == "code")])
            if len(nb_code) > 0:
                new_test_scripts.append("def test_nb_integration_{}():\n".format(nb_idx) +
